...
- Additional targets: `mSceneMap` (SSR input), `mLuminanceMap`/`mExposureMap` (auto-exposure), `postPingMap`/`postPongMap` (post-process ping-pong), `mFXAAMap`, `mSMAABlendBuffer`, `bloomMip[BLOOM_MAX_MIPS]` (HDR bloom pyramid, up to 7 levels, live count in `bloomMipCount`), `crossFilter[3]` (cross-screen filter scratch and accumulator, allocated lazily on the first frame the effect is enabled and released when it is switched off — `crossFilterHeight` guards the state), `mWaterDis` (refraction), `mSpotShadow[2]`, `mPbrBrdfLut`
