...
S32 LLScrollListCtrl::getItemCount() const
{
    if (mIsFiltered)
    {
        ...
        for(LLScrollListItem* item : mItemList)
        {
            ...
            LLStringUtil::toLower(filterColumnValue);
...
S32 LLScrollListCtrl::calcMaxContentWidth()
{
    ...
    for (LLScrollListColumn* column : mColumnsIndexed)
    {
        ...
        if (mColumnWidthsDirty)
        {
            ...
            column->mMaxContentWidth = column->mHeader ? LLFontGL::getFontSansSerifSmall()->getWidthBytes(column->mLabel.getString(), 0, S32_MAX) + mColumnPadding + HEADING_TEXT_PADDING : 0;
            for (LLScrollListItem* item : mItemList)
            {
...
U32 LLScrollListCtrl::searchItems(const std::string& substring, bool case_sensitive, bool focus)
{
    ...
    else
    {
        ...
        {
            // Only select enabled items with matching names
            if (!item->getEnabled() || isFiltered(item))
            {
                continue;
            }
...
void LLScrollListCtrl::copy()
{
    ...
    LLClipboard::instance().copyToClipboard(buffer, 0, static_cast<S32>(buffer.length()));
}
