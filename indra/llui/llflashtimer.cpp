    ...
    mUnset(false)
{
	...
	mFlashCount = 2 * ((count > 0) ? count : LLUI::getInstanceFast()->mSettingGroups["config"]->getS32("FlashCount"));
	if (mPeriod <= 0)
	{
		mPeriod = LLUI::getInstanceFast()->mSettingGroups["config"]->getF32("FlashPeriod");
	}
}
