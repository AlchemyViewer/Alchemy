...
function(al_feature_summary)
  ...
  feature_summary(WHAT PACKAGES_NOT_FOUND QUIET_ON_EMPTY DESCRIPTION "Packages not found:")
endfunction()
...
function(al_configuration_report)
  ...
  message(
    STATUS
    "${AL_CHANNEL} ${VIEWER_SHORT_VERSION}.${VIEWER_VERSION_REVISION} (revision from ${VIEWER_VERSION_REVISION_SOURCE})"
  )
  ...
  if(AL_USE_SENTRY)
    if(AL_ENABLE_CRASH_REPORTING)
      _al_report_row("Crash reports" "Sentry, sending enabled")
    else()
      _al_report_row("Crash reports" "Sentry, sending disabled")
    endif()
  else()
  ...
  if(DEFINED AL_BUILD_PACKAGE AND NOT AL_BUILD_PACKAGE)
    _al_report_row("Package" "none (AL_BUILD_PACKAGE off)")
  else()
    if(LINUX)
      set(package_kind "tar.xz")
    elseif(DARWIN)
      set(package_kind "dmg")
    else()
      set(package_kind "zip")
    endif()
    if(AL_USE_VELOPACK)
      string(APPEND package_kind ", Velopack")
    endif()
