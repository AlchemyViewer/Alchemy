...
file(CONFIGURE OUTPUT "${CMAKE_CURRENT_BINARY_DIR}/package.env" CONTENT
...
if(AL_USE_VELOPACK)
  ...
  if(DARWIN)
    set(velopack_args
      --packDir "${velopack_dir}/app/${AL_INSTALL_BUNDLE}"
      --mainExe "${al_velopack_main_exe}"
      --bundleId "${MACOSX_BUNDLE_GUI_IDENTIFIER}"
      --icon "${BRANDING_SOURCE_DIR}/viewer/icons/${ICON_PATH}/alchemy.icns"
      --noInst)
    set(velopack_rename "")
  else()
    set(velopack_args
      --packDir "${velopack_dir}/app"
      --mainExe "${al_velopack_main_exe}"
      --icon "${BRANDING_SOURCE_DIR}/installer/icons/install_icon.ico"
      --splashImage "${BRANDING_SOURCE_DIR}/installer/splash/${ICON_PATH}/install_splash.gif"
      --splashProgressColor "${al_velopack_splash_color}"
      --shortcuts "")
    set(velopack_rename
      COMMAND ${CMAKE_COMMAND} -E rename "${velopack_releases}/${AL_APP_NAME_ONEWORD}-win-Setup.exe" "${velopack_releases}/${AL_PACKAGE_NAME}_Setup.exe"
      COMMAND ${CMAKE_COMMAND} -E rename "${velopack_releases}/${AL_APP_NAME_ONEWORD}-win-Portable.zip" "${velopack_releases}/${AL_PACKAGE_NAME}_Portable.zip")
  endif()
  add_custom_target(velopack
    COMMAND ${CMAKE_COMMAND} -E rm -rf "${velopack_dir}"
    COMMAND ${CMAKE_COMMAND} --install "${CMAKE_BINARY_DIR}" --config $<CONFIG> --prefix "${velopack_dir}/app"
    COMMAND dotnet vpk pack
      --packId "${AL_APP_NAME_ONEWORD}"
      --packVersion "${al_velopack_version}"
      --packAuthors "${al_velopack_authors}"
      --packTitle "${al_velopack_title}"
      --outputDir "${velopack_releases}"
      ${velopack_args}
    ${velopack_rename}
