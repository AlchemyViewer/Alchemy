...
install(TARGETS ${AL_VIEWER_BINARY_NAME} COMPONENT viewer
  RUNTIME DESTINATION "${AL_INSTALL_BINDIR}"
  BUNDLE DESTINATION .)

if(WINDOWS)
  install(FILES $<TARGET_RUNTIME_DLLS:${AL_VIEWER_BINARY_NAME}>
    DESTINATION "${AL_INSTALL_BINDIR}" COMPONENT viewer)
endif()

if(TARGET llwebrtc)
  install(TARGETS llwebrtc COMPONENT viewer ${al_release_configurations}
    RUNTIME DESTINATION "${AL_INSTALL_BINDIR}"
...
if(AL_BUILD_SHARED_LLCOMMON)
  install(TARGETS llcommon COMPONENT viewer
    RUNTIME DESTINATION "${AL_INSTALL_BINDIR}"
    LIBRARY DESTINATION "${AL_INSTALL_LIBDIR}"
    ARCHIVE DESTINATION lib COMPONENT devel EXCLUDE_FROM_ALL)
  if(WINDOWS)
    install(TARGETS llcommon COMPONENT plugins
...
if(NOT AL_USE_NSSPELLCHECKER AND NOT AL_USE_WINSPELLCHECK)
  if(WINDOWS)
    ...
    install(FILES ${al_hunspell_libraries}
      DESTINATION "${AL_INSTALL_LIBDIR}" COMPONENT viewer)
  else()
    install(FILES ${hunspell_LINK_LIBRARIES} FOLLOW_SYMLINK_CHAIN
      DESTINATION "${AL_INSTALL_LIBDIR}" COMPONENT viewer)
  endif()
endif()
if(AL_USE_OPENAL AND NOT WINDOWS)
  install(FILES $<TARGET_SONAME_FILE:OpenAL::OpenAL> FOLLOW_SYMLINK_CHAIN
    DESTINATION "${AL_INSTALL_LIBDIR}" COMPONENT viewer)
endif()

# The SDKs from outside vcpkg ship the library the viewer loads at run time.
if(AL_USE_FMODSTUDIO AND FMOD_LIBRARY_RELEASE)
  install(FILES "$<IF:$<CONFIG:Debug>,${FMOD_LIBRARY_DEBUG},${FMOD_LIBRARY_RELEASE}>" FOLLOW_SYMLINK_CHAIN
    DESTINATION "${AL_INSTALL_LIBDIR}" COMPONENT viewer)
endif()
if(AL_USE_DISCORD)
  if(WINDOWS)
    install(FILES "${DISCORD_SDK_RUNTIME_DIR}/discord_partner_sdk.dll"
      DESTINATION "${AL_INSTALL_LIBDIR}" COMPONENT viewer)
  elseif(DARWIN)
    install(FILES "${DISCORD_SDK_RUNTIME_DIR}/libdiscord_partner_sdk.dylib"
      DESTINATION "${AL_INSTALL_LIBDIR}" COMPONENT viewer)
  else()
    file(GLOB al_discord_libraries "${DISCORD_SDK_RUNTIME_DIR}/libdiscord_partner_sdk.so*")
    install(FILES ${al_discord_libraries}
      DESTINATION "${AL_INSTALL_LIBDIR}" COMPONENT viewer)
  endif()
endif()
...
if(AL_USE_BUGSPLAT)
  if(WINDOWS)
    install(PROGRAMS "${al_vcpkg_dir}/tools/BsSndRpt64.exe"
      DESTINATION "${AL_INSTALL_BINDIR}" COMPONENT viewer)
    install(FILES "${al_vcpkg_dir}/bin/BugSplat64.dll" "${al_vcpkg_dir}/bin/BugSplatRc64.dll"
      DESTINATION "${AL_INSTALL_BINDIR}" COMPONENT viewer)
  elseif(DARWIN)
    install(DIRECTORY "${al_vcpkg_dir}/lib/BugSplat.framework"
      DESTINATION "${AL_INSTALL_LIBDIR}" USE_SOURCE_PERMISSIONS COMPONENT viewer)
  endif()
endif()
...
foreach(plugin media_plugin_cef media_plugin_libvlc media_plugin_gstreamer10)
  if(TARGET ${plugin})
    ...
    install(TARGETS ${plugin} COMPONENT plugins ${configurations}
      RUNTIME DESTINATION "${AL_INSTALL_PLUGINDIR}"
      LIBRARY DESTINATION "${AL_INSTALL_PLUGINDIR}"
      BUNDLE DESTINATION "${AL_INSTALL_PLUGINDIR}")
    if(WINDOWS)
      install(FILES $<TARGET_RUNTIME_DLLS:${plugin}>
        DESTINATION "${AL_INSTALL_PLUGINDIR}" ${configurations} COMPONENT plugins)
    endif()
  endif()
endforeach()
...
if(TARGET media_plugin_example AND NOT AL_CHANNEL_TYPE STREQUAL "release")
  install(TARGETS media_plugin_example COMPONENT plugins
    RUNTIME DESTINATION "${AL_INSTALL_PLUGINDIR}"
    BUNDLE DESTINATION "${AL_INSTALL_PLUGINDIR}")
endif()
...
if(TARGET media_plugin_cef)
  if(WINDOWS)
    ...
    install(PROGRAMS "${cef_binary_dir}/bootstrap.exe" RENAME media_plugin_cef.exe
      DESTINATION "${AL_INSTALL_PLUGINDIR}" COMPONENT cef)
    install(TARGETS dullahan_host COMPONENT cef
      RUNTIME DESTINATION "${AL_INSTALL_PLUGINDIR}")
    install(FILES $<TARGET_RUNTIME_DLLS:dullahan_host>
      DESTINATION "${AL_INSTALL_PLUGINDIR}" COMPONENT cef)
    install(FILES
  ...
  elseif(DARWIN)
    ...
    install(DIRECTORY "${CEF_FRAMEWORK_DIR}"
      DESTINATION "${cef_frameworks_dir}" USE_SOURCE_PERMISSIONS COMPONENT cef)
    install(TARGETS dullahan_host dullahan_host_alerts dullahan_host_gpu dullahan_host_plugin dullahan_host_renderer
      COMPONENT cef BUNDLE DESTINATION "${cef_frameworks_dir}")
  else()
    ...
    install(TARGETS dullahan_host COMPONENT cef
      RUNTIME DESTINATION "${AL_INSTALL_PLUGINDIR}")
    install(PROGRAMS "${CEF_BINARY_DIR_RELEASE}/chrome-sandbox"
      DESTINATION "${AL_INSTALL_PLUGINDIR}" COMPONENT cef)
    install(FILES
    ...
    install(DIRECTORY "${CEF_RESOURCE_DIR}/locales"
      DESTINATION "${AL_INSTALL_PLUGINDIR}" COMPONENT cef)
    install(FILES $<TARGET_FILE:unofficial::cef::libcef>
      "${CEF_BINARY_DIR_RELEASE}/v8_context_snapshot.bin"
      "${CEF_BINARY_DIR_RELEASE}/vk_swiftshader_icd.json"
      "${CEF_RESOURCE_DIR}/chrome_100_percent.pak"
      "${CEF_RESOURCE_DIR}/chrome_200_percent.pak"
      "${CEF_RESOURCE_DIR}/resources.pak"
      "${CEF_RESOURCE_DIR}/icudtl.dat"
      DESTINATION "${AL_INSTALL_LIBDIR}" COMPONENT cef)
    install(DIRECTORY "${CEF_BINARY_DIR_RELEASE}/"
      DESTINATION "${AL_INSTALL_LIBDIR}" COMPONENT cef
      FILES_MATCHING
      PATTERN "libEGL*"
      PATTERN "libGLESv2*"
...
if(TARGET media_plugin_libvlc)
  ...
  if(WINDOWS)
    install(DIRECTORY "${VLC_PLUGINS_DIR}/"
      DESTINATION "${AL_INSTALL_PLUGINDIR}/plugins" ${al_release_configurations} COMPONENT plugins)
  elseif(DARWIN)
    ...
    install(FILES ${vlc_libraries}
      DESTINATION "${vlc_frameworks_dir}" COMPONENT plugins)
    install(DIRECTORY "${VLC_PLUGINS_DIR}/"
      DESTINATION "${vlc_frameworks_dir}/plugins" COMPONENT plugins
      FILES_MATCHING PATTERN "*.dylib" PATTERN "plugins.dat")
  endif()
endif()
...
install(DIRECTORY "${al_newview_dir}/app_settings/"
...
foreach(tree shaders camera windlight filters colorlut looks poses)
  install(DIRECTORY "${al_newview_dir}/app_settings/${tree}"
    DESTINATION "${AL_INSTALL_DATADIR}/app_settings" COMPONENT viewer)
endforeach()
install(FILES "${SCRIPTS_DIR}/messages/message_template.msg"
  DESTINATION "${AL_INSTALL_DATADIR}/app_settings" COMPONENT viewer)
install(DIRECTORY "${al_vcpkg_dir}/share/alchemy-dictionaries/dictionaries"
  DESTINATION "${AL_INSTALL_DATADIR}/app_settings" COMPONENT viewer)
install(DIRECTORY "${LSL_DEFINITIONS_DIR}/"
  DESTINATION "${AL_INSTALL_DATADIR}/app_settings/syntax_default" COMPONENT viewer
  FILES_MATCHING
  PATTERN "*.txt"
  PATTERN "*.xml"
  PATTERN "*.json"
...
install(DIRECTORY "${al_newview_dir}/character/"
  DESTINATION "${AL_INSTALL_DATADIR}/character" COMPONENT viewer
  FILES_MATCHING
  PATTERN "*.llm"
  PATTERN "*.xml"
  PATTERN "*.tga")

install(DIRECTORY "${ALCHEMY_FONTS_DIR}/"
  DESTINATION "${AL_INSTALL_DATADIR}/fonts" COMPONENT viewer
  FILES_MATCHING
  PATTERN "*.otf"
  PATTERN "*.ttc"
  PATTERN "*.ttf"
...
install(DIRECTORY "${al_newview_dir}/skins/"
  DESTINATION "${AL_INSTALL_DATADIR}/skins" COMPONENT viewer
  FILES_MATCHING
  PATTERN "*.xml"
  PATTERN "*.json"
  PATTERN "*.png"
...
install(FILES "${al_newview_dir}/cube.dae"
  DESTINATION "${AL_INSTALL_DATADIR}" COMPONENT viewer)
install(FILES "${INDRA_SOURCE_DIR}/externals/ca-certificates/ca-bundle.crt"
  DESTINATION "${al_ca_bundle_dir}" COMPONENT viewer)
...
if(WINDOWS AND AL_USE_VELOPACK)
  ...
  install(FILES
    ...
    DESTINATION . COMPONENT velopack)
endif()

if(DARWIN)
  install(FILES "${BRANDING_SOURCE_DIR}/viewer/icons/${ICON_PATH}/alchemy.icns"
    DESTINATION "${AL_INSTALL_DATADIR}" COMPONENT viewer)
  install(DIRECTORY "${al_newview_dir}/cursors_mac"
    DESTINATION "${AL_INSTALL_DATADIR}" COMPONENT viewer
    FILES_MATCHING PATTERN "*.tif")
  install(FILES
    "${al_newview_dir}/English.lproj/language.txt"
    "${CMAKE_CURRENT_BINARY_DIR}/InfoPlist.strings"
    DESTINATION "${AL_INSTALL_DATADIR}/English.lproj" COMPONENT viewer)
  foreach(lproj German Japanese Korean da es fr hu it nl pl pt ru tr uk zh-Hans)
    install(DIRECTORY "${al_newview_dir}/${lproj}.lproj"
      DESTINATION "${AL_INSTALL_DATADIR}" COMPONENT viewer)
  endforeach()
  ...
  if(NOT DEFINED ENV{GITHUB_ACTIONS})
    install(CODE "set(AL_SIGN_BUNDLE \"\${CMAKE_INSTALL_PREFIX}/${AL_INSTALL_BUNDLE}\")
...
if(LINUX)
  install(PROGRAMS "${al_newview_dir}/linux_tools/wrapper.sh" RENAME alchemy
    DESTINATION . COMPONENT viewer)
  install(PROGRAMS "${al_newview_dir}/linux_tools/install.sh"
    DESTINATION . COMPONENT viewer)
  install(PROGRAMS
    ...
    DESTINATION etc COMPONENT viewer)
  install(DIRECTORY "${al_newview_dir}/res-sdl"
    DESTINATION . COMPONENT viewer)
  install(FILES "${BRANDING_SOURCE_DIR}/viewer/icons/${ICON_PATH}/alchemy_256.png" RENAME alchemy_icon.png
    DESTINATION . COMPONENT viewer)
endif()
