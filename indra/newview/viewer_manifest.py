...
class ViewerManifest(LLManifest):
    ...
    def construct(self):
        ...
        definitions_dir = os.path.join(self.args['vcpkg_dir'], 'share', 'lsl-definitions')
        with self.prefix(src=os.path.join(definitions_dir, 'lsl_definitions'), dst="app_settings/syntax_default"):
            self.path("*.txt")
            self.path("*.yaml")
            self.path("*.xml")
            self.path("*.json")
...
class LinuxManifest(ViewerManifest):
    ...
    def construct(self):
        ...
        with self.prefix(src=os.path.join(self.args['vcpkg_dir'], 'share', 'cef-bin', 'Release'), dst="lib"):
            self.path( "libEGL*" )
            self.path( "libvulkan*" )
        ...
        with self.prefix(src=os.path.join(self.args['vcpkg_dir'], 'share', 'cef-bin', 'Resources'), dst="lib"):
