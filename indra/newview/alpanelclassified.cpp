/**
 * @file alpanelclassified.cpp
 * @brief ALPanelClassifiedInfo class implementation
 *
...
//static
ALPanelClassifiedInfo::panel_list_t ALPanelClassifiedInfo::sAllPanels;
static LLPanelInjector<ALPanelClassifiedInfo> t_panel_al_classified_info("panel_al_classified_info");
...
public:
        ...
        const sparam_t& strings)
    {
        ...
        ALPanelClassifiedInfo::setClickThrough(
...
static LLPanelInjector<ALPanelClassifiedInfo> t_al_classified_info("panel_al_classified_info");

ALPanelClassifiedInfo::ALPanelClassifiedInfo()
 : LLPanel()
 , mInfoLoaded(false)
 , mFromSearch(false)
...
// static
ALPanelClassifiedInfo* ALPanelClassifiedInfo::create()
{
    ALPanelClassifiedInfo* panel = new ALPanelClassifiedInfo();
    panel->buildFromFile("panel_al_classified_info.xml");
    return panel;
}

bool ALPanelClassifiedInfo::postBuild()
{
    childSetAction("back_btn", boost::bind(&ALPanelClassifiedInfo::onExit, this));
    childSetAction("show_on_map_btn", boost::bind(&ALPanelClassifiedInfo::onMapClick, this));
    childSetAction("teleport_btn", boost::bind(&ALPanelClassifiedInfo::onTeleportClick, this));
...
void ALPanelClassifiedInfo::setExitCallback(const commit_callback_t& cb)
{
    getChild<LLButton>("back_btn")->setClickedCallback(cb);
}

void ALPanelClassifiedInfo::setEditClassifiedCallback(const commit_callback_t& cb)
{
    getChild<LLButton>("edit_btn")->setClickedCallback(cb);
}

void ALPanelClassifiedInfo::reshape(S32 width, S32 height, bool called_from_parent /* = true */)
{
...
void ALPanelClassifiedInfo::onOpen(const LLSD& key)
{
    ...
    if (gAgent.getRegion())
    {
        ...
        if (!url.empty())
        {
            ...
            LLCoreHttpUtil::HttpCoroutineAdapter::callbackHttpPost(url, body,
                boost::bind(&ALPanelClassifiedInfo::handleSearchStatResponse, classifiedId, _1));
        }
    }
...
/*static*/
void ALPanelClassifiedInfo::handleSearchStatResponse(LLUUID classifiedId, LLSD result)
{
    ...
    ALPanelClassifiedInfo::setClickThrough(classifiedId,
...
void ALPanelClassifiedInfo::processProperties(void* data, EAvatarProcessorType type)
{
...
void ALPanelClassifiedInfo::resetData()
{
...
void ALPanelClassifiedInfo::resetControls()
{
...
void ALPanelClassifiedInfo::setClassifiedName(const std::string& name)
{
    getChild<LLUICtrl>("classified_name")->setValue(name);
}

std::string ALPanelClassifiedInfo::getClassifiedName()
{
    return getChild<LLUICtrl>("classified_name")->getValue().asString();
}

void ALPanelClassifiedInfo::setDescription(const std::string& desc)
{
    getChild<LLUICtrl>("classified_desc")->setValue(desc);
}

std::string ALPanelClassifiedInfo::getDescription()
{
    return getChild<LLUICtrl>("classified_desc")->getValue().asString();
}

void ALPanelClassifiedInfo::setClassifiedLocation(const std::string& location)
{
    getChild<LLUICtrl>("classified_location")->setValue(location);
}

std::string ALPanelClassifiedInfo::getClassifiedLocation()
{
    return getChild<LLUICtrl>("classified_location")->getValue().asString();
}

void ALPanelClassifiedInfo::setSnapshotId(const LLUUID& id)
{
    mSnapshotCtrl->setValue(id);
    mSnapshotStreched = false;
}

void ALPanelClassifiedInfo::draw()
{
    LLPanel::draw();
...
LLUUID ALPanelClassifiedInfo::getSnapshotId()
{
    return getChild<LLUICtrl>("classified_snapshot")->getValue().asUUID();
}

// static
void ALPanelClassifiedInfo::setClickThrough(
    ...
    bool from_new_table)
{
    ...
    for (auto self : sAllPanels)
    {
        ...
        if (typeid(*self) != typeid(ALPanelClassifiedInfo))
        {
            continue;
        }
...
// static
std::string ALPanelClassifiedInfo::createLocationText(
...
void ALPanelClassifiedInfo::stretchSnapshot()
{
    // *NOTE dzaporozhan
...
LLRect ALPanelClassifiedInfo::getDefaultSnapshotRect()
{
...
void ALPanelClassifiedInfo::scrollToTop()
{
...
void ALPanelClassifiedInfo::sendClickMessage(
...
void ALPanelClassifiedInfo::sendClickMessage(const std::string& type)
{
    sendClickMessage(
        type,
        fromSearch(),
...
void ALPanelClassifiedInfo::onMapClick()
{
...
void ALPanelClassifiedInfo::onTeleportClick()
{
...
void ALPanelClassifiedInfo::onExit()
{
    LLAvatarPropertiesProcessor::getInstance()->removeObserver(getAvatarId(), this);
