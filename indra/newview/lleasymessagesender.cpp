    ...
    */
    ...
    case MVT_VARIABLE:
        if(!hex)
        {
            ...
            gMessageSystem->addBinaryData(var_name, input.c_str(), narrow(input.size() + 1));
        }
        else
        {
            gMessageSystem->addBinaryData(var_name, input.c_str(), narrow(input.size()));
        }
        return TRUE;
        break;
    case MVT_FIXED:
