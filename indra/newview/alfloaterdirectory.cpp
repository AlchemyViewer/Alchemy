...
ALFloaterDirectory::ALFloaterDirectory(const Params& key)
    : LLFloater(key)
...
{
    mCommitCallbackRegistrar.add("Search.NavigateResults", boost::bind(&ALFloaterDirectory::navigateResults, this, _2));
    mCommitCallbackRegistrar.add("Search.Popout", boost::bind(&ALFloaterDirectory::onCommitPopoutResult, this));
}

ALFloaterDirectory::~ALFloaterDirectory()
{
    if (mAvatarNameCallbackConnection.connected()) { mAvatarNameCallbackConnection.disconnect(); }
}

bool ALFloaterDirectory::postBuild()
{
    mResultList = findChild<LLScrollListCtrl>("results");
    mResultList->setCommitCallback(boost::bind(&ALFloaterDirectory::onCommitSelection, this));
    for (std::string panel_name: sSearchPanels)
    {
        ALPanelSearch* panel = static_cast<ALPanelSearch*>(findChild<LLUICtrl>(panel_name));
        if (panel) { panel->setSearchFloater(this); }
    }
    mPanelWeb = findChild<ALPanelSearchWeb>("panel_search_web");
    mTabContainer = findChild<LLTabContainer>("search_tabs");
    mTabContainer->setCommitCallback(boost::bind(&ALFloaterDirectory::onTabChanged, this));
    setProgress(false);
...
void ALFloaterDirectory::onOpen(const LLSD& key)
{
    Params p(key);
...
void ALFloaterDirectory::onClose(bool app_quitting)
{
    if (mTabContainer)
    {
...
void ALFloaterDirectory::setProgress(bool working)
{
    getChild<LLUICtrl>("loading")->setVisible(working);
}

void ALFloaterDirectory::setResultsComment(const std::string& message) const
{
    mResultList->setCommentText(message);
}

void ALFloaterDirectory::onTabChanged()
{
...
void ALFloaterDirectory::onCommitSelection()
{
    LLSD params;
...
void ALFloaterDirectory::paginate()
{
...
void ALFloaterDirectory::navigateResults(const LLSD& userdata)
{
...
void ALFloaterDirectory::showDetailPanel(const std::string& panel_name)
{
...
void ALFloaterDirectory::rebuildResultList() const
{
...
void ALFloaterDirectory::onCommitPopoutResult()
{
...
void ALFloaterDirectory::queryAvatarKey(const LLDirQuery& query)
{
    ...
    mAvatarNameCallbackConnection = LLAvatarNameCache::get(LLUUID(query.text),
            boost::bind(&ALFloaterDirectory::onAvatarNameCallback, this, _1, _2));
}

void ALFloaterDirectory::onAvatarNameCallback(const LLUUID& id, const LLAvatarName& av_name)
{
    if (mAvatarNameCallbackConnection.connected())
    {
        mAvatarNameCallbackConnection.disconnect();
...
void ALFloaterDirectory::queryGroupKey(const LLDirQuery& query)
{
    if (mCurrentResultType != query.type)
    {
        mCurrentResultType = query.type;
    ...
    gCacheName->getGroup(LLUUID(query.text), // i love you my sexy boost::bind.
        boost::bind(&ALFloaterDirectory::onGroupNameCallback, this, _1, _2, _3));
    // lambda expressions will never take your place in my heart. <3
}

void ALFloaterDirectory::onGroupNameCallback(const LLUUID& id, const std::string& name, bool is_group)
{
    setProgress(false);
...
void ALFloaterDirectory::queryDirectory(const LLDirQuery& query, bool new_search)
{
    if (mCurrentResultType != query.type)
    {
        mCurrentResultType = query.type;
...
//static
void ALFloaterDirectory::processSearchPeopleReply(LLMessageSystem* msg, void**)
{
    LLUUID query_id;
    ...
    ALFloaterDirectory* self = LLFloaterReg::findTypedInstance<ALFloaterDirectory>("search");
...
//static
void ALFloaterDirectory::processSearchGroupsReply(LLMessageSystem* msg, void**)
{
    LLUUID query_id;
    LLUUID group_id;
    LLUUID agent_id;
...
//static
void ALFloaterDirectory::processSearchPlacesReply(LLMessageSystem* msg, void**)
{
    LLUUID  agent_id;
    LLUUID  query_id;
    LLUUID  parcel_id;
...
//static
void ALFloaterDirectory::processSearchClassifiedsReply(LLMessageSystem* msg, void**)
{
    LLUUID  agent_id;
    LLUUID  query_id;
...
// static
void ALFloaterDirectory::processSearchLandReply(LLMessageSystem* msg, void**)
{
    LLUUID  agent_id;
    LLUUID  query_id;
    LLUUID  parcel_id;
...
// static
void ALFloaterDirectory::processSearchEventsReply(LLMessageSystem* msg, void**)
{
    LLUUID  agent_id;
    LLUUID  query_id;
    LLUUID  owner_id;
