/**
 * @file alpanelclassifiededit.cpp
 * @brief ALPanelClassifiedEdit class definition
 *
...
ALPanelClassifiedEdit::ALPanelClassifiedEdit() :
    ALPanelClassifiedInfo(),
    mIsNew(false),
...
// static
ALPanelClassifiedEdit* ALPanelClassifiedEdit::create()
{
    ALPanelClassifiedEdit* panel = new ALPanelClassifiedEdit();
    panel->buildFromFile("panel_al_edit_classified.xml");
    return panel;
}

bool ALPanelClassifiedEdit::postBuild()
{
    ALPanelClassifiedInfo::postBuild();

    LLUICtrl* edit_icon = getChild<LLUICtrl>("edit_icon");
    mSnapshotCtrl->setMouseEnterCallback(boost::bind(&ALPanelClassifiedEdit::onTexturePickerMouseEnter, this, edit_icon));
    mSnapshotCtrl->setMouseLeaveCallback(boost::bind(&ALPanelClassifiedEdit::onTexturePickerMouseLeave, this, edit_icon));
    edit_icon->setVisible(false);

    getChild<LLLineEditor>("classified_name")->setKeystrokeCallback(boost::bind(&ALPanelClassifiedEdit::onChange, this), nullptr);
    getChild<LLTextEditor>("classified_desc")->setKeystrokeCallback(boost::bind(&ALPanelClassifiedEdit::onChange, this));
    ...
    combobox->setCommitCallback(boost::bind(&ALPanelClassifiedEdit::onChange, this));

    childSetCommitCallback("content_type", boost::bind(&ALPanelClassifiedEdit::onChange, this), nullptr);
    childSetCommitCallback("price_for_listing", boost::bind(&ALPanelClassifiedEdit::onChange, this), nullptr);
    childSetCommitCallback("auto_renew", boost::bind(&ALPanelClassifiedEdit::onChange, this), nullptr);

    childSetAction("save_changes_btn", boost::bind(&ALPanelClassifiedEdit::onSaveClick, this));
    childSetAction("set_to_curr_location_btn", boost::bind(&ALPanelClassifiedEdit::onSetLocationClick, this));

    mSnapshotCtrl->setOnSelectCallback(boost::bind(&ALPanelClassifiedEdit::onTextureSelected, this));

    return true;
}

void ALPanelClassifiedEdit::fillIn(const LLSD& key)
{
...
void ALPanelClassifiedEdit::onOpen(const LLSD& key)
{
    ...
    {
        ALPanelClassifiedInfo::onOpen(key);
    }
...
void ALPanelClassifiedEdit::processProperties(void* data, EAvatarProcessorType type)
{
    if (APT_CLASSIFIED_INFO == type)
    {
        ...
        {
            // see ALPanelClassifiedEdit::sendUpdate() for notes
            mIsNewWithErrors = false;
            ...
            // *HACK see ALPanelClassifiedEdit::sendUpdate()
...
bool ALPanelClassifiedEdit::isDirty() const
{
    if (mIsNew)
    {
        return true;
    ...
    dirty |= ALPanelClassifiedInfo::isDirty();
...
void ALPanelClassifiedEdit::resetDirty()
{
    ALPanelClassifiedInfo::resetDirty();
...
void ALPanelClassifiedEdit::setSaveCallback(const commit_signal_t::slot_type& cb) { mSaveButtonClickedSignal.connect(cb); }

void ALPanelClassifiedEdit::setCancelCallback(const commit_signal_t::slot_type& cb)
{
    getChild<LLButton>("cancel_btn")->setClickedCallback(cb);
}

void ALPanelClassifiedEdit::resetControls()
{
    ALPanelClassifiedInfo::resetControls();
...
void ALPanelClassifiedEdit::draw()
{
    LLPanel::draw();
...
void ALPanelClassifiedEdit::stretchSnapshot()
{
    ALPanelClassifiedInfo::stretchSnapshot();

    getChild<LLUICtrl>("edit_icon")->setShape(mSnapshotCtrl->getRect());
}

U32 ALPanelClassifiedEdit::getContentType()
{
    LLComboBox* ct_cb = getChild<LLComboBox>("content_type");
    return ct_cb->getCurrentIndex();
}

void ALPanelClassifiedEdit::setContentType(U32 content_type)
{
    LLComboBox* ct_cb = getChild<LLComboBox>("content_type");
...
bool ALPanelClassifiedEdit::getAutoRenew() { return getChild<LLUICtrl>("auto_renew")->getValue().asBoolean(); }

void ALPanelClassifiedEdit::sendUpdate()
{
...
U32 ALPanelClassifiedEdit::getCategory()
{
    LLComboBox* cat_cb = getChild<LLComboBox>("category");
    return cat_cb->getCurrentIndex();
}

void ALPanelClassifiedEdit::setCategory(U32 category)
{
    LLComboBox* cat_cb = getChild<LLComboBox>("category");
...
U8 ALPanelClassifiedEdit::getFlags()
{
...
void ALPanelClassifiedEdit::enableVerbs(bool enable) { getChildView("save_changes_btn")->setEnabled(enable); }

void ALPanelClassifiedEdit::enableEditing(bool enable)
{
...
void ALPanelClassifiedEdit::showEditing(bool show)
{
    getChildView("price_for_listing_label")->setVisible(show);
    getChildView("price_for_listing")->setVisible(show);
}

std::string ALPanelClassifiedEdit::makeClassifiedName()
{
    std::string name;
...
S32 ALPanelClassifiedEdit::getPriceForListing() { return getChild<LLUICtrl>("price_for_listing")->getValue().asInteger(); }

void ALPanelClassifiedEdit::setPriceForListing(S32 price) { getChild<LLUICtrl>("price_for_listing")->setValue(price); }

void ALPanelClassifiedEdit::onSetLocationClick()
{
    setPosGlobal(gAgent.getPositionGlobal());
    setParcelId(LLUUID::null);
...
void ALPanelClassifiedEdit::onChange() { enableVerbs(isDirty()); }

void ALPanelClassifiedEdit::onSaveClick()
{
    mCanClose = false;
    ...
    if (isNew() || isNewWithErrors())
    {
        ...
        if (!mPublishFloater)
        {
            ...
            mPublishFloater->setPublishClickedCallback(boost::bind(&ALPanelClassifiedEdit::onPublishFloaterPublishClicked, this));
        }
...
void ALPanelClassifiedEdit::doSave()
{
    mCanClose = true;
    sendUpdate();
    resetDirty();
...
void ALPanelClassifiedEdit::onPublishFloaterPublishClicked()
{
...
bool ALPanelClassifiedEdit::isValidName()
{
...
void ALPanelClassifiedEdit::notifyInvalidName()
{
    const std::string& name = getClassifiedName();
    if (name.empty())
    {
