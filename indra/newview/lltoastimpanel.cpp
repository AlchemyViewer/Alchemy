...
// [SL:KB] - Patch: Chat-Alerts | Checked: 2012-08-29 (Catznip-3.3)
    ...
    {
        // Append the name first without chat-alert parsing so the actor's name can't
        // satisfy a keyword. The body that follows runs through chat-alert parsing.
    ...
    {
        const S32 nHighlightMask = mMessage->getHighlightsMask();
        ...
        mMessage->setHighlightsMask(nMask);
        mMessage->appendText(message, false, style_params);
        mMessage->setHighlightsMask(nHighlightMask);
    }
    else
    {
        mMessage->appendText(message, false, style_params);
