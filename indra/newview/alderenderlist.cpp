...
ALDerenderEntry::ALDerenderEntry(EEntryType eType, bool fPersists)
...
ALDerenderEntry::ALDerenderEntry(EEntryType eType, const LLSD& sdData)
    : m_eType(TYPE_NONE)
{
    // Sanity check
...
std::unique_ptr<ALDerenderEntry> ALDerenderEntry::fromLLSD(const LLSD& sdData)
{
    EEntryType eType = (sdData.has("entry_type")) ? (EEntryType)sdData["entry_type"].asInteger() : TYPE_OBJECT;
    switch (eType)
    {
        case TYPE_OBJECT:
            return std::make_unique<ALDerenderObject>(sdData);
        case TYPE_AVATAR:
            return std::make_unique<ALDerenderAvatar>(sdData);
        default:
            return nullptr;
    }
}

LLSD ALDerenderEntry::toLLSD() const
{
    LLSD sdData;
...
ALDerenderObject::ALDerenderObject(const LLSelectNode* pNode, bool fPersist)
    : ALDerenderEntry(TYPE_OBJECT, fPersist), idRegion(0), idRootLocal(0)
{
    //
...
ALDerenderObject::ALDerenderObject(const LLSD& sdData)
    : ALDerenderEntry(TYPE_OBJECT, sdData), idRegion(0), idRootLocal(0)
{
...
LLSD ALDerenderObject::toLLSD() const
{
    LLSD sdData = ALDerenderEntry::toLLSD();
...
ALDerenderList::change_signal_t ALDerenderList::s_ChangeSignal;
std::string ALDerenderList::s_PersistFilename = "derender_list.llsd";

ALDerenderList::ALDerenderList()
{
    load();
}
...
void ALDerenderList::load()
{
    ...
    for (const LLSD& sdEntry : llsd::inArray(inSD))
    {
        auto pEntry = ALDerenderEntry::fromLLSD(sdEntry);
        if (pEntry->isValid())
            m_Entries.push_back(std::move(pEntry));
    }
}

void ALDerenderList::save() const
{
...
ALDerenderList::entry_list_t::iterator ALDerenderList::findEntry(ALDerenderEntry::EEntryType eType, const LLUUID& idEntry)
{
    return std::find_if(m_Entries.begin(), m_Entries.end(), [eType, &idEntry](const auto& e) { return (eType == e->getType()) && (idEntry == e->getID()); });
}

ALDerenderList::entry_list_t::iterator ALDerenderList::findObjectEntry(U64 idRegion, const LLUUID& idObject, U32 idRootLocal)
{
    ...
    return std::find_if(m_Entries.begin(), m_Entries.end(),
                        [&idRegion, &idObject, &idRootLocal](const std::unique_ptr<ALDerenderEntry>& e)
                        { return (ALDerenderEntry::TYPE_OBJECT == e->getType()) &&
                                 ( ((idRootLocal) && (idRegion == ((ALDerenderObject*)e.get())->idRegion) && (idRootLocal == ((ALDerenderObject*)e.get())->idRootLocal)) || (idObject == e->getID()) ); });
}

void ALDerenderList::removeObject(ALDerenderEntry::EEntryType eType, const LLUUID& idObject)
{
...
void ALDerenderList::removeObjects(ALDerenderEntry::EEntryType eType, const uuid_vec_t& idsObject)
{
    bool fSave = false;
    ...
    for (const auto& idObject : idsObject)
    {
        ...
        switch (eType)
        {
            case ALDerenderEntry::TYPE_OBJECT:
                {
                    ALDerenderObject* pObjEntry = static_cast<ALDerenderObject*>(pEntry.get());
                ...
                break;
            default:
            case ALDerenderEntry::TYPE_AVATAR:
            case ALDerenderEntry::TYPE_NONE:
                break;
        }
...
bool ALDerenderList::addSelection(bool fPersist, std::vector<LLUUID>* pIdList)
{
    if (pIdList)
    {
        pIdList->clear();
    ...
    while (hSel->root_end() != itObj)
    {
        ...
        auto uniqueEntry = std::make_unique<ALDerenderObject>(pNode, fPersist);
...
bool ALDerenderList::canAddSelection()
{
    struct CanDerender : public LLSelectedObjectFunctor
    {
        /*virtual*/ bool apply(LLViewerObject* pObj) { return ALDerenderList::canAdd(pObj); }
    } f;
    LLObjectSelectionHandle hSel = LLSelectMgr::getInstance()->getSelection();
    return (hSel.notNull()) && (0 != hSel->getRootObjectCount()) && (hSel->applyToRootObjects(&f, false));
}

ALDerenderObject* ALDerenderList::getObjectEntry(const LLUUID& idObject) /*const*/
{
    if (m_Entries.empty()) return nullptr;
    ALDerenderList::entry_list_t::iterator itEntry = findEntry(ALDerenderEntry::TYPE_OBJECT, idObject);
    return (m_Entries.end() != itEntry) ? (ALDerenderObject*)itEntry->get() : nullptr;
}

ALDerenderObject* ALDerenderList::getObjectEntry(U64 idRegion, const LLUUID& idObject, U32 idRootLocal) /*const*/
{
    if (m_Entries.empty()) return nullptr;
    ALDerenderList::entry_list_t::const_iterator itEntry = findObjectEntry(idRegion, idObject, idRootLocal);
    return (m_Entries.end() != itEntry) ? (ALDerenderObject*)itEntry->get() : nullptr;
}

bool ALDerenderList::processObjectUpdate(U64 idRegion, const LLUUID& idObject, const LLVOCacheEntry* pCacheEntry)
{
    // Used by OUT_FULL_CACHED
    if (!pCacheEntry->isChild())
    {
        // We know it's a root prim so we can skip looking up its parent
        ALDerenderObject* pObjEntry = getObjectEntry(idObject);
        if (!pObjEntry)
        {
...
bool ALDerenderList::processObjectUpdate(U64 idRegion, const LLUUID& idObject, U32 idObjectLocal, U32 idRootLocal)
{
    // Used by OUT_FULL
    ALDerenderObject* pObjEntry = (0 == idRootLocal) ? getObjectEntry(idObject) : getObjectEntry(idRegion, idObject, idRootLocal);
    if (pObjEntry)
    {
        if (0 != idRootLocal)
        {
...
bool ALDerenderList::processObjectUpdate(U64 idRegion, const LLUUID& idObject, U32 idObjectLocal, const U8* pBuffer)
{
