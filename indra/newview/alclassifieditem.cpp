...
ALClassifiedItem::ALClassifiedItem(const LLUUID& avatar_id, const LLUUID& classified_id)
    : LLPanel()
    , mAvatarId(avatar_id)
    , mClassifiedId(classified_id)
{
    buildFromFile("panel_al_classifieds_list_item.xml");

    LLAvatarPropertiesProcessor::getInstance()->addObserver(getAvatarId(), this);
    LLAvatarPropertiesProcessor::getInstance()->sendClassifiedInfoRequest(getClassifiedId());
}

ALClassifiedItem::~ALClassifiedItem()
{
    LLAvatarPropertiesProcessor::getInstance()->removeObserver(getAvatarId(), this);
}

void ALClassifiedItem::processProperties(void* data, EAvatarProcessorType type)
{
...
bool ALClassifiedItem::postBuild()
{
...
void ALClassifiedItem::setValue(const LLSD& value)
{
...
void ALClassifiedItem::fillIn(ALPanelClassifiedEdit* panel)
{
    if (!panel)
    {
        return;
...
void ALClassifiedItem::setClassifiedName(const std::string& name)
{
    getChild<LLUICtrl>("name")->setValue(name);
}

void ALClassifiedItem::setDescription(const std::string& desc)
{
    getChild<LLUICtrl>("description")->setValue(desc);
}

void ALClassifiedItem::setSnapshotId(const LLUUID& snapshot_id)
{
    getChild<LLUICtrl>("picture")->setValue(snapshot_id);
}

LLUUID ALClassifiedItem::getSnapshotId() const
{
