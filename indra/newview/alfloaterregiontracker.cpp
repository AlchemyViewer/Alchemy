...
const std::string TRACKER_FILE = "tracked_regions.xml";
...
bool ALFloaterRegionTracker::saveToFile()
{
	...
	if (out_file.is_open())
	{
		LLSDSerialize::toPrettyXML(mRegionMap, out_file);
		out_file.close();
		return true;
	}
	return false;
...
bool ALFloaterRegionTracker::loadFromFile()
{
	const std::string& filename = gDirUtilp->getExpandedFilename(LL_PATH_USER_SETTINGS, TRACKER_FILE);
	llifstream in_file;
	...
	if (in_file.is_open())
	{
		LLSDSerialize::fromXML(mRegionMap, in_file);
		in_file.close();
		return true;
	}
	return false;
