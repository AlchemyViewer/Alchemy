...
,   mDate(0)
{
    mCommitCallbackRegistrar.add("Search.query", boost::bind(&ALPanelSearchEvents::onCommitSearch, this, _1));
    mCommitCallbackRegistrar.add("Search.AddDay", boost::bind(&ALPanelSearchEvents::addDay, this));
    mCommitCallbackRegistrar.add("Search.MinusDay", boost::bind(&ALPanelSearchEvents::minusDay, this));
}

bool ALPanelSearchEvents::postBuild()
{
    mSearchEditor = getChild<LLSearchEditor>("search_bar");
    //mSearchEditor->setKeystrokeCallback(boost::bind(&ALPanelSearchEvents::onCommitSearch, this, _1));
    setDate(0);

    return LLPanel::postBuild();
}

void ALPanelSearchEvents::onCommitSearch(LLUICtrl* ctrl)
{
...
void ALPanelSearchEvents::search()
{
    LLDirQuery query;
...
void ALPanelSearchEvents::setDate(S32 day)
{
    mDate = day;
...
void ALPanelSearchEvents::addDay()
{
    setDate(++mDate);
...
void ALPanelSearchEvents::minusDay()
{
    setDate(--mDate);
    if (childGetValue("events_search_mode").asString() == "date")
        search();
