/*
 * @file alfloatertransactionlog.h
...
ALFloaterTransactionLog::ALFloaterTransactionLog(const LLSD& key) :
    LLFloater(key),
    mList(nullptr),
...
{
    mCommitCallbackRegistrar.add("TL.Reset", boost::bind(&ALFloaterTransactionLog::reset, this));
}

ALFloaterTransactionLog::~ALFloaterTransactionLog()
{
    if (mAvatarNameCacheConnection.connected())
        mAvatarNameCacheConnection.disconnect();
}

bool ALFloaterTransactionLog::postBuild()
{
    mList = getChild<LLScrollListCtrl>("transaction_list");
    mList->setDoubleClickCallback(boost::bind(&ALFloaterTransactionLog::onDoubleClick, this));
...
void ALFloaterTransactionLog::addTransaction(const LLDate& date, const LLUUID& sender, S32 amount, bool incoming)
{
    ...
    mAvatarNameCacheConnection =
        LLAvatarNameCache::get(sender, boost::bind(&ALFloaterTransactionLog::onAvatarNameCache, this, _1, _2, row));
}

void ALFloaterTransactionLog::onAvatarNameCache(const LLUUID& agent_id, const LLAvatarName& av_name, LLSD& row)
{
...
void ALFloaterTransactionLog::onDoubleClick()
{
    const LLUUID& id = mList->getFirstSelected()->getValue().asUUID();
    LLAvatarActions::showProfile(id);
}

void ALFloaterTransactionLog::reset()
{
...
void ALFloaterTransactionLog::updateLabels()
{
