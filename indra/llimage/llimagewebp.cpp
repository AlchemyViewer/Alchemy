...
bool LLImageWebP::encode(const LLImageRaw* raw_image, F32 encode_time)
{
    ...
    if (components == 4)
    {
        encodedSize = WebPEncodeLosslessRGBA(tmp_buff.get(), width, height, narrow(stride), &encodedData);
    }
    else
    {
        encodedSize = WebPEncodeLosslessRGB(tmp_buff.get(), width, height, narrow(stride), &encodedData);
    }
    ...
    if (!allocateData(narrow(encodedSize)))
    {
